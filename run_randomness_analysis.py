...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./plots/randomness_analysis",
...
def create_bias_summary_table(df_decisions, granularity_level, debug=False):
    ...
    if not all(column in df_decisions.columns for column in required_columns):
        if debug:
            print(
                f"Missing required columns in df_decisions. Required: {required_columns}"
            )
        return pd.DataFrame()  # Return an empty DataFrame
    ...
    model_types = {"OLMo-Tulu": [], "OLMo-Flan": [], "T5-Tulu": [], "T5-Flan": []}
    ...
    model_bias_scores = df_decisions.pivot_table(
        values="individual_score", index="model", columns="bias", aggfunc="mean"
    )
    ...
    model_bias_scores["Average_Abs_Bias"] = (
        model_bias_scores.drop("Average_Bias", axis=1).abs().mean(axis=1)
    )
...
def add_mmlu_scores(df_results, mmlu_mapping=None, debug=False):
    ...
    for model in df_results["Model"].unique():
        if model in mmlu_mapping:
            df_results.loc[df_results["Model"] == model, "MMLU"] = mmlu_mapping[model][
                "score"
            ]
            df_results.loc[df_results["Model"] == model, "MMLU_std"] = mmlu_mapping[
                model
            ]["std"]
...
def add_certainty_belief_bias(df_results, debug=False):
    ...
    if "Certainty Bias" not in df_results.columns:
        ...
        df_results["Certainty Bias_std"] = (
            0.01 + np.random.random(len(df_results)) * 0.05
        )
    ...
    if "Belief Valid Bias" not in df_results.columns:
        ...
        df_results["Belief Valid Bias_std"] = (
            0.01 + np.random.random(len(df_results)) * 0.05
        )
...
def export_to_csv(df_results, output_dir, granularity_level, debug=False):
    ...
    filename = os.path.join(
        output_dir, "tables", f"model_bias_results_{granularity_level}.csv"
    )
...
def export_to_latex(df_results, output_dir, granularity_level, debug=False):
    ...
    # Create output filename
    filename = os.path.join(
        output_dir, "tables", f"model_bias_results_{granularity_level}.tex"
    )
...
def create_heatmap_from_results(df_results, output_dir, granularity_level, debug=False):
    ...
    # Save the figure
    filename = os.path.join(
        output_dir, "plots", f"model_bias_heatmap_{granularity_level}.pdf"
    )
...
def create_std_heatmap(df_results, output_dir, granularity_level, debug=False):
    ...
    std_columns = [
        col for col in df_results.columns if col.endswith("_std") and col != "MMLU_std"
    ]
    ...
    std_data = std_data.rename(
        columns={
            std_col: base_col for std_col, base_col in zip(std_columns, base_columns)
        }
    )
    ...
    # Save the figure
    filename = os.path.join(
        output_dir, "plots", f"model_bias_std_heatmap_{granularity_level}.pdf"
    )
    plt.savefig(filename, format="pdf", bbox_inches="tight")
...
def load_decision_data_for_randomness_analysis(exclude_random, error_threshold):
    ...
    """
    # Load decision data only from models of cross tunning (OLMo and T5 based)
    models_to_include = [
        model_name
        for model_name in MODELS_TO_INCLUDE
        if "OLMo" in model_name or "T5" in model_name
    ]
    print(f"Models to include: {models_to_include}")
...
def main():
    ...
    # Load decision data
    df_decisions = load_decision_data_for_randomness_analysis(
        args.exclude_random, args.error_threshold
    )
    ...
    for granularity_level in args.granularity_levels:
        ...
        df_results = create_bias_summary_table(
            df_decisions, granularity_level, args.debug
        )
        ...
        if args.save_plots:
            ...
            if source_path.exists():
                target_path = (
                    Path(output_dir) / "plots" / f"bias_heatmap_{granularity_level}.pdf"
                )
                import shutil
