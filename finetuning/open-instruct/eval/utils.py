                ...
                for output_idx in range(batch_outputs.shape[0]):
                    for token_idx in range(
                        ...
                        if any(
                            ...
                            batch_outputs[output_idx, token_idx:] = (
...
def get_next_word_predictions(
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        # if model is not t5 just get the logits
        if "t5" not in model.config.model_type:
            batch_logits = model(
                input_ids=batch_input_ids, attention_mask=attention_mask
            ).logits[:, -1, :]
        else:
            # if model is t5, we need to generate the next token
            batch_logits = model.generate(
                input_ids=batch_input_ids,
                attention_mask=attention_mask,
                # max_length=batch_input_ids.shape[1] + 1,
                max_new_tokens=1,
...
def score_completions(
    ...
    for i in range(0, len(unrolled_examples), batch_size):
        batch_prompts = [
            example["prompt"] for example in unrolled_examples[i : i + batch_size]
        ]
        batch_examples = [
            (
                example["prompt"]
                if example["prompt"][-1] in ["\n", " "]
                else example["prompt"] + " "
            )
            + example["completion"]
        ...
        for example_idx, (prompt, example) in enumerate(
            zip(batch_prompts, batch_examples)
        ):
            tokenized_prompt = tokenizer(
                prompt, padding=False, return_tensors="pt"
            ).input_ids.squeeze(0)
            tokenized_example = tokenizer(
                example, padding=False, return_tensors="pt"
            ).input_ids.squeeze(0)
            ...
            if tokenizer.padding_side == "right":
                example_logits = outputs.logits[
                    example_idx, : len(tokenized_example), :
                ]
            else:
                example_logits = outputs.logits[
                    example_idx, -len(tokenized_example) :, :
            ...
            completion_logits = example_logits[
                len(tokenized_prompt) - 1 : len(tokenized_example) - 1, :
            ]
...
def load_hf_tokenizer(
    ...
    try:
        if "t5" not in model_name_or_path:
        ...
        if "t5" not in model_name_or_path:
            tokenizer = AutoTokenizer.from_pretrained(
                tokenizer_name_or_path, token=token
            )
        else:
            tokenizer = AutoTokenizer.from_pretrained(
                tokenizer_name_or_path, token=token, legacy=False
            )
