...
class PopulationError(Exception):
    """
    Exception raised during the population of test case templates.
    
    This exception provides detailed context about template population failures,
    including the original template and model output for debugging.
    """
    ...
    def __init__(
        ...
        """
        Initialize PopulationError with context information.

        Args:
            message (str): The error message describing what went wrong.
            template (Template): The template that failed to populate.
            model_output (str): The raw model output that caused the failure.
        """
        ...
        if template is not None:
            try:
                ...
                logger.warning("Could not include template in error message")
...
class DecisionError(Exception):
    """
    Exception raised during the decision-making process of test cases.
    
    This exception is raised when an LLM fails to make a decision on a test case
    or when the decision extraction process fails.
    """
    pass
...
class MetricCalculationError(Exception):
    """
    Exception raised during the calculation of bias metrics.
    
    This exception is raised when there are issues computing bias scores
    from test results, such as invalid data or calculation errors.
    """
    pass
...
class LLM(ABC):
    ...
    def __init__(
        ...
        """
        Initialize the LLM with option randomization settings.

        Args:
            randomly_flip_options (bool): Whether to randomly flip answer options in 50% of cases.
            shuffle_answer_options (bool): Whether to randomly shuffle all answer options.
        """
        ...
        logger.debug(f"Initialized LLM with NAME={self.NAME}, flip_options={randomly_flip_options}, shuffle_options={shuffle_answer_options}")
            ...
            for test_id, test_case in enumerate(test_cases):
                ...
                for retry in range(max_retries):
                    try:
                        ...
                        # if the test case is decided successfully, break the retry loop and increment the seed to the next position                        seed += max_retries - retry
                        break
                    except Exception as e:
                        number_of_errors += 1
                        current_error = e
                        ...
                        if max_retries == 1:
                            print(
                                f"Warning: the model {self.NAME} failed to make a decision on the test case {test_id}.\nError: {current_error}\nSkipping..."
                            )
                ...
                if test_decision is None:
                    ...
                    if max_retries > 1:
                        print(
                            f"Max retries of {max_retries} reached for the test case {test_id} by the model {self.NAME}.\nSkipping..."
                        )
                all_decisions.append(test_decision)
                # update every 100 decisions
                if test_id % 100 == 0:
                    pbar.update(100)

        # print warning if there are any errors
        if number_of_errors > 0:
            print("\n" + "=" * 100 + "\n")
            print(
                f"WARNING: {number_of_errors} errors occurred while making decisions for the model {self.NAME}."
            )
            print(f"Number of errors: {number_of_errors} out of {len(test_cases)}")
            print("=" * 100 + "\n")
        return all_decisions, number_of_errors

    def _validate_population(
        self, template: Template, insertions: dict, raw_model_output: str = None
    ) -> bool:
        """
        ...
        for gap in gaps:
            if gap not in insertions:
                raise PopulationError(
                    f"The gap '{gap}' has not been filled.", template, raw_model_output
                )
        ...
        for pattern in insertions.keys():
            if pattern not in gaps:
                raise PopulationError(
                    f"An insertion was generated for a non-existing gap '{pattern}'.",
                    template,
                    raw_model_output,
                )
        ...
        for pattern in insertions.keys():
            if insertions[pattern] == None or insertions[pattern].strip() == "":
                raise PopulationError(
                    f"Invalid insertion '{insertions[pattern]}' attempted into gap '{pattern}'. Insertion is empty.",
                    template,
                    raw_model_output,
                )
            ...
            stripped_insertion = (
                insertions[pattern].strip("[[").strip("]]").strip("{{").strip("}}")
            )
            if stripped_insertion == stripped_pattern:
                raise PopulationError(
                    f"Invalid insertion '{insertions[pattern]}' attempted into gap '{pattern}'. Insertion is identical with the gap instruction.",
                    template,
                    raw_model_output,
                )
...
class TestGenerator(ABC):
    ...
    def sample_custom_values(
        self, num_instances: int = 5, iteration_seed: int = 42
    ) -> dict:
        """
    ...
    def generate_all(
        ...
        for scenario in scenarios:
            ...
            iteration_seed = int(hashlib.md5(scenario.encode()).hexdigest(), 16) % (
                2**32
            )
            ...
            for step in range(num_instances):
                ...
                sampled_values_step = self.select_custom_values_for_step(
                    sampled_values, step
                )
                for retry in range(max_retries):
                    try:
    ...
    def load_config(self, bias: str) -> TestConfig:
        ...
        """
        return TestConfig(f"./tests/{bias.title().replace(' ', '')}/config.xml")
    ...
    def populate(
        ...
        control, treatment = model.populate(
            control, treatment, scenario, temperature, seed
        )
...
class RatioScaleMetric:
    ...
    def __init__(
        self,
        test_results: list[tuple[TestCase, DecisionResult]],
        k: np.array = np.array([-1]),
        x_1: np.array = np.array([0]),
        x_2: np.array = np.array([0]),
        test_weights: np.array = np.array([1]),
        flip_control: bool = False,
        flip_treatment: bool = False,
    ):
    ...
    def _compute(
        ...
        """
        delta_control_abs, delta_treatment_abs = np.abs(
            control_answer - self.x_1
        ), np.abs(treatment_answer - self.x_2)
        metric_value = (
            self.k
            * (delta_control_abs - delta_treatment_abs)
            / (np.maximum(delta_control_abs, delta_treatment_abs) + 1e-8)
        )
...
class NominalScaleMetric:
    ...
    def __init__(
        self,
        test_results: list[tuple[TestCase, DecisionResult]],
        options_labels: np.array = np.empty(0),
    ...
    def _compute(
        ...
        if self.f == "abs":
            ...
            raise MetricCalculationError(
                f"Unknown function '{self.f}' in the metric calculation."
            )
