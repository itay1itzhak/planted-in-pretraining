...
MODEL_DEVELOPER_MAPPING = {
    ...
    "Mistral-Tulu-Seed-0": "Mistral-Tulu",
    "Mistral-Tulu-Seed-1": "Mistral-Tulu",  # different learning rate
    "Mistral-ShareGPT": "Mistral-ShareGPT",
    "Llama2-Tulu": "Llama2-Tulu",
    "Llama2-ShareGPT-Seed-0": "Llama2-ShareGPT",
    "Llama2-ShareGPT-Seed-1": "Llama2-ShareGPT",
    "Llama2-ShareGPT-Seed-2": "Llama2-ShareGPT",
}
...
def load_model_bias_data(
    df_decisions: pd.DataFrame = None, df_tests: pd.DataFrame = None
) -> pd.DataFrame:
    """
    ...
    df = df.pivot_table(
        ...
        aggfunc=lambda x: list(
            x
        ),  # Merge rows into a list to preserve order of multiple values
    )

    # Step 4: Split the lists into 5 individual rows
    df = df.reset_index()  # Reset index before exploding
    df = df.melt(id_vars=["model", "scenario"], var_name="bias", value_name="score")
    df = df.explode("score")
    df = df.reset_index(drop=True)  # Clean up the index after exploding
...
def reduce_with_umap(df: pd.DataFrame, n_components: int = 2):
    ...
    umap_result = pd.DataFrame(
        umap_reducer.fit_transform(df[num_cols]),
...
def plot_bias_heatmap(
    ...
    sns.heatmap(
        ...
        cmap=sns.diverging_palette(
            220, 20, as_cmap=True
        ),  # Use a red-white-blue color palette
        ...
        vmin=-1.0,
        vmax=1.0,  # Ensure the color scale covers the range [-1.0, 1.0]
    ...
    ax.hlines(
        [heatmap_data.shape[0] - 1], *ax.get_xlim(), color="white", linewidth=5
    )  # Row gap
    if add_avg_abs:
        ax.hlines(
            [heatmap_data.shape[0] - 2], *ax.get_xlim(), color="white", linewidth=5
        )  # Row gap
    ax.vlines(
        [heatmap_data.shape[1] - 1], *ax.get_ylim(), color="white", linewidth=5
    )  # Column gap
    ...
    if legend:
        ...
        cbar = plt.colorbar(
            ax.collections[0], cax=cbar_ax, orientation="horizontal", label="Bias"
        )
...
def plot_scatter(
    ...
    if len(num_cols) != 2:
        raise ValueError(
            f"DataFrame must contain exactly two numeric columns. Found {len(num_cols)}."
        )
    ...
    scatter = plt.scatter(
        x=df[num_cols[0]], y=df[num_cols[1]], c=colors, s=dot_size, alpha=dot_alpha
    )
    ...
    handles = [
        plt.Line2D(
            ...
            [0],
            marker="o",
            color=plt.cm.Spectral(i / len(unique_labels)),
            markersize=10,
            linestyle="",
        )
    ...
    plt.legend(
        handles, unique_labels, title=label, bbox_to_anchor=(1.05, 1), loc="upper left"
    )
    ...
    if save_plot:
        plt.savefig(
            os.path.join(PLOT_OUTPUT_FOLDER, "cluster_plot.pdf"),
            format="pdf",
            bbox_inches="tight",
        )
...
def plot_dendrogram(
    ...
    if save_plot:
        plt.savefig(
            os.path.join(PLOT_OUTPUT_FOLDER, "dendrogram.pdf"),
            format="pdf",
            bbox_inches="tight",
        )
...
def plot_correlation_matrix(df: pd.DataFrame, save_plot: bool = True):
    ...
    if save_plot:
        plt.savefig(
            os.path.join(PLOT_OUTPUT_FOLDER, "bias_correlation.pdf"),
            format="pdf",
            bbox_inches="tight",
        )
