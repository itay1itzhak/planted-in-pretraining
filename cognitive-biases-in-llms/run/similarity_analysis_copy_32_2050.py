...
def add_certainty_and_belief_to_pivot_df(
    ...
    """
    Adds certainty and belief columns to the pivot_df. These are manually added rows in the file.
...
def get_feature_matrix_for_clustering(
    ...
    pivot_df.T.to_csv(f"../plots/clustering/debug_pivot_df_{level}.csv")
...
# ):
#     """
#     Creates visualization of clustering results.
#     """
#     # For sample-level analysis, adjust PCA parameters
#     n_components = 2
#     if granularity_level == 'model_bias_sample' and feature_matrix.shape[1] > 100:
#         n_components = min(10, feature_matrix.shape[1] // 10)

#     # Scale features if requested
#     features = StandardScaler().fit_transform(feature_matrix) if with_scaling else feature_matrix

#     # Reduce dimensionality with PCA
#     pca = PCA(n_components=n_components)
#     coords_initial = pca.fit_transform(features)

#     # If we used more than 2 components, do a second PCA for visualization
#     if n_components > 2:
#         pca_final = PCA(n_components=2)
#         coords = pca_final.fit_transform(coords_initial)
#     else:
...
#     if n_components > 2:
#         plt.xlabel(f'PC1-2 projection (from {n_components} components)')
#         plt.ylabel(f'PC2-2 projection (from {n_components} components)')
#     else:
...
def plot_clustering_results(
    ...
    def create_scatter_plot(
        ...
        # cluster_edge_colors = ['#2C3E50', '#8E44AD']  # Dark blue and purple for cluster edges
    ...
    #     + [
    #         # Instruction type legend (markers)
    #         Line2D([0], [0], marker=m, color="gray", label=t, markersize=15)
    #         for t, m in [
    #             ("Tulu", "o"),
    #             ("Flan", "^"),
    #             ("ShareGPT", "s"),
    ...
    #             markeredgewidth=4,
    #         )
    #         for i, c in enumerate(cluster_edge_colors)  # Using consistent colors
    #     ]
    # )
...
def plot_similarity_matrix(
    ...
    #     'pretraining_group': ['OLMo' if 'OLMo' in name else 'T5' for name in model_metadata.index]
    # }).set_index('model')
    model_metadata = pd.DataFrame(
        {
            "model": model_metadata.index,
...
def perform_unsupervised_clustering_analysis(
    ...
    # Add new analyses
    # feature_analysis = analyze_cluster_features(
    #     feature_matrix if not with_scaling else scaled_features,
    #     labels,
    #     model_metadata
    # )

    # similarity_analysis = analyze_model_similarities(
    #     feature_matrix if not with_scaling else scaled_features,
    #     labels,
    #     model_metadata
    # )
    ...
    #     for cluster_id, analysis in feature_analysis.items():
    #         print(f"\nCluster {cluster_id} ({analysis['size']} models):")
    #         print("Models:", ", ".join(analysis['models']))
    #         print("\nMost distinctive features:")
    #         for feature, importance in analysis['distinctive_features'].items():
    #             print(f"  {feature}: {importance:.3f}")

    #     print("\nSimilarity Analysis:")
    #     print("\nWithin-cluster similarities:")
    #     for cluster_id, stats in similarity_analysis['within_cluster'].items():
    #         print(f"\nCluster {cluster_id}:")
    #         print(f"Mean similarity: {stats['mean_similarity']:.3f} (±{stats['std_similarity']:.3f})")

    #     print("\nBetween-cluster similarities:")
    #     for (c1, c2), stats in similarity_analysis['between_cluster'].items():
    #         print(f"\nClusters {c1} - {c2}:")
    #         print(f"Mean similarity: {stats['mean_similarity']:.3f} (±{stats['std_similarity']:.3f})")
    #         print("Most similar pairs:")
    #         for pair in stats['closest_pairs']:
    #             print(f"  {pair['models'][0]} - {pair['models'][1]}: {pair['similarity']:.3f}")
...
# def compute_similarity_statistics(feature_matrix: pd.DataFrame) -> dict:
#     """
#     Computes similarity statistics for bias vectors.

#     Args:
#         feature_matrix: DataFrame with bias features for each model.

#     Returns:
#         similarity_results: Dictionary with similarity statistics.
#     """
...
#     instruction_similarities = []
#     pretraining_similarities = []
#     cross_instruction_similarities = []
#     cross_pretraining_similarities = []

#     # Get model metadata
#     model_metadata = assign_model_groups(pd.DataFrame({'model': feature_matrix.index}))

#     # Iterate over all pairs of models
#     for i, model_i in enumerate(feature_matrix.index):
#         for j, model_j in enumerate(feature_matrix.index):
#             if i >= j:
#                 continue  # Avoid duplicate pairs and self-comparison

#             # Check if models share the same instruction dataset
#             if model_metadata.loc[model_i, 'instruction_group'] == model_metadata.loc[model_j, 'instruction_group']:
#                 instruction_similarities.append(cosine_sim[i, j])
#             else:
#                 cross_instruction_similarities.append(cosine_sim[i, j])

#             # Check if models share the same pretrained model
#             if model_metadata.loc[model_i, 'pretraining_group'] == model_metadata.loc[model_j, 'pretraining_group']:
#                 pretraining_similarities.append(cosine_sim[i, j])
#             else:
#                 cross_pretraining_similarities.append(cosine_sim[i, j])

#     # Calculate mean similarities
#     mean_instruction_similarity = np.mean(instruction_similarities)
#     mean_pretraining_similarity = np.mean(pretraining_similarities)

#     # Perform statistical tests
#     instruction_p_value = ttest_ind(instruction_similarities, cross_instruction_similarities, equal_var=False).pvalue
#     pretraining_p_value = ttest_ind(pretraining_similarities, cross_pretraining_similarities, equal_var=False).pvalue

#     # Create similarity matrices
#     instruction_similarity_matrix = pd.DataFrame(instruction_similarities, index=feature_matrix.index, columns=feature_matrix.index)
#     pretraining_similarity_matrix = pd.DataFrame(pretraining_similarities, index=feature_matrix.index, columns=feature_matrix.index)

#     # Compile results
#     similarity_results = {
#         'mean_instruction_similarity': mean_instruction_similarity,
#         'mean_pretraining_similarity': mean_pretraining_similarity,
#         'instruction_p_value': instruction_p_value,
#         'pretraining_p_value': pretraining_p_value,
#         'cosine_similarity_feature_matrix': cosine_sim,
#     }
