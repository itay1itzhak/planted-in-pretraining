...
def decide_batch(batch: pd.DataFrame, model_name: str, model, randomly_flip_options: bool, shuffle_answer_options: bool, temperature: float, seed: int):
    """
    ...
    for bias in biases:
        ...
        decision_results, number_of_errors = model.decide_all(test_cases, temperature, seed, max_retries=1)
        errors_in_each_bias[bias] = number_of_errors
    ...
    # Return the errors in each bias
    return errors_in_each_bias
...
def decide_dataset(dataset: pd.DataFrame, model_name: str, n_batches: int, n_workers: int, randomly_flip_options: bool, shuffle_answer_options: bool, temperature: float, seed: int):
    ...
    if n_workers > 1:
        ...
        unified_errors_in_each_bias = {}
        # Decide the dataset batch by batch
        with tqdm(total=len(batches)) as progress_bar:
            for batch in batches:
                errors_in_each_bias = decide_batch(batch, model_name, model, randomly_flip_options, shuffle_answer_options, temperature, seed)
                # Unify the errors in each bias considering the same bias may have been decided in different batches
                for bias, number_of_errors in errors_in_each_bias.items():
                    unified_errors_in_each_bias[bias] = unified_errors_in_each_bias.get(bias, 0) + number_of_errors
                progress_bar.update()

    # Save the errors in each bias to a CSV file
    errors_df = pd.DataFrame(unified_errors_in_each_bias.items(), columns=["bias", "number_of_errors"])
    errors_df.to_csv(os.path.join(DECISION_RESULTS, model_name, f"errors_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv"), index=False)
...
def main():
    ...
    parser.add_argument("--max_retries", type=int, help="The maximum number of retries to make for a decision.", default=1)
